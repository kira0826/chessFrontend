...
const MoveNavigation: React.FC<MoveNavigationProps> = ({
  ...
  const formatGain = (gain: number): string => {
    const formattedGain = gain.toFixed(2);
        ...
        <table className="w-full">
          <thead className="sticky top-0 bg-gray-200">
            <tr>
              ...
              <th className="py-2 text-right pr-4">Gain</th>
            </tr>
          </thead>
          <tbody>
            {Array.from({ length: Math.ceil(plays.length / 2) }).map((_, i) => (
              <tr key={i} className="hover:bg-gray-300">
                ...
                >
                  {plays[i * 2] && `${plays[i * 2].origin}-${plays[i * 2].destination}`}
                </td>
                <td
                  className={`py-2 cursor-pointer ${
                ...
                >
                  {plays[i * 2 + 1] && `${plays[i * 2 + 1].origin}-${plays[i * 2 + 1].destination}`}
                </td>
                <td className="py-2 text-right pr-4">
                  {plays[i * 2] && formatGain(plays[i * 2].gain)}
                </td>
              </tr>
            ))}
          </tbody>
